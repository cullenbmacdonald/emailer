...
struct EmailStoreTests {
    ...
    @Test("setActionQueuePagination updates state")
    func setPagination() {
        let store = EmailStore()
    ...
    @Test("appendActionQueueEmails adds without duplicates")
    func appendEmails() {
        let store = EmailStore()
    ...
    @Test("selectedDetail defaults to nil")
    func selectedDetailDefault() {
        let store = EmailStore()
        #expect(store.selectedDetail == nil)
    }

    @Test("clearDetail sets nil")
    func clearDetail() {
        let store = EmailStore()
        store.clearDetail()
        #expect(store.selectedDetail == nil)
    ...
    @Test("loadDetail with nil client does nothing")
