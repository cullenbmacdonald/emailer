...
struct EmailRowViewTests {
    ...
    func timestampNow() {
        let email = makeEmail(receivedAt: Date().addingTimeInterval(-30))
        let formatter = TestTimestampFormatter()
        let result = formatter.relativeTimestamp(for: email.receivedAt)
        #expect(result == "now")
    }

    @Test("Timestamp shows minutes for recent emails")
    func timestampMinutes() {
        let email = makeEmail(receivedAt: Date().addingTimeInterval(-120))
        let formatter = TestTimestampFormatter()
        let result = formatter.relativeTimestamp(for: email.receivedAt)
        #expect(result == "2m")
    }
    ...
    func timestampHours() {
        let email = makeEmail(receivedAt: Date().addingTimeInterval(-7200))
        let formatter = TestTimestampFormatter()
        let result = formatter.relativeTimestamp(for: email.receivedAt)
